...
const PlayVideo = () => {
    ...
    const fetchVideoData = async () => {
        ...
        await fetch(videoDetails_url)
            .then(response => response.json()).then(data => setApiData(data.items[0]))
    }
    const fetchOtherData = async () => {
        ...
        const comments_url = `https://www.googleapis.com/youtube/v3/commentThreads?part=snippet,replies&videoId=${videoId}&key=${API_KEY}`
        await fetch(comments_url).then(response => response.json()).then(data =>setCommentData(data.items))
    }
  ...
  return (
    <div className="play-video">
        ...
        <div className="vid-discription">
            <p>{apiData?apiData.snippet.description.slice(0,250):"Lorem ipsum dolor sit amet consectetur adipisicing elit. Doloribus, cumque."}</p>
